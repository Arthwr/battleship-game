...
describe("Player Model", () => {
  ...
  test("should correctly return move set for computer chooseMove method", () => {
    const opponentBoard = new Gameboard(10, 10);
    const computerPlayer = new ComputerPlayer(new Gameboard(10, 10));
    const move = computerPlayer.chooseMove(opponentBoard);
  ...
  test("chooseMove should avoid repeating moves", () => {
    ...
    const move1 = computerPlayer.chooseMove(gameBoard);
    ...
    jest.spyOn(global.Math, "random").mockReturnValueOnce(0);
    const move2 = computerPlayer.chooseMove(gameBoard);
